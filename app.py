...
# --- 설정: JWT 및 보안 설정 ---
SECRET_KEY = os.environ.get('SECRET_KEY', 'your_super_secret_key_change_me') # 실제 환경에서는 환경 변수를 사용해야 합니다.
JWT_ALGORITHM = 'HS256'
ACCESS_TOKEN_EXPIRE_MINUTES = 30
REFRESH_TOKEN_EXPIRE_DAYS = 7 # 여기서는 refresh token을 구현하지 않지만, 개념적으로 추가
...
# --- 2. 백그라운드 센서 데이터 저장 (기존 server.py) ---
...
# --- 3. API 엔드포인트 정의 (기존 arduinoController.js + server.py) ---
...
def logout():
    ...
    return jsonify({"status": "success", "message": "Logout successful. Refresh token invalidated."}), 200
...
def get_ssd_temp(device_name="sda"):
    ...
    try:
        ...
        output = subprocess.check_output(
            ["sudo", "smartctl", "-A", f"/dev/{device_name}"],
        ...
        # NVMe 장치를 위한 대체 검색
        match_nvme = re.search(r"Temperature Sensor \d+:\s*(\d+)\s*C", output, re.IGNORECASE)
        if match_nvme:
